...
def save(path, logger, epoch_id, model,model_blip,optimizer, scheduler, global_step, tb_logger, default_gpu, score, is_best=False):
    if default_gpu:
        ...
        save_obj = {
                'model': model_blip.state_dict(),
                'epoch': epoch_id,
            }
        output_model_blip_file = os.path.join(path, "blip_model_" + str(epoch_id) + ".pth")
        torch.save(save_obj, output_model_blip_file) 
        
        if is_best:
            output_model_file = os.path.join(path, "pytorch_model_best"+".bin")
            output_model_blip_file = os.path.join(path, "blip_model_best"+".pth")
            save_obj = {
                'model': model_blip.state_dict(),
                'epoch': epoch_id,
            }
        ...
        blip_output_checkpoint = os.path.join(path, "blip_ckpt_latest.tar")
        save_obj = {
            'model': model_blip.state_dict(),
            'epoch': epoch_id,
        }
        torch.save(model_to_save.state_dict(), blip_output_checkpoint)
